%% 박사논문 민감도 분석 알고리즘 - Matlab
% 목적:
% 1. 예산, 연간 시공력, SAIDI 상한, Risk 총량 상한 변화에 따른 투자효과를 분석한다.
% 2. 투자가치 기반 ILP, Local PI 기반 ILP, Integrated PI 기반 ILP를 같은 조건에서 비교한다.
% 3. 논문 5.9절에 사용할 연도별 결과, 합산 결과, 설비군별 결과, 제약 충족 여부, 그래프를 생성한다.
%
% 실행:
%   run("code/sensitivity_analysis_matlab.m")
%
% 선택 환경변수:
%   SENS_FULL_GRID=1              : 예산 x 물량 x SAIDI x Risk 전체 격자 분석
%   SENS_ILP_MAX_TIME=0           : ILP 시간 제한 제거
%   SENS_ILP_MAX_TIME=600         : ILP 시나리오별 최대 600초
%   SENS_ILP_REL_GAP=0.001        : 상대 갭 0.1%
%   SENS_METHODS=investment_value_ilp,local_pi_ilp,integrated_pi_ilp
%   SENS_SCOPES=risk_value_screening,local_pi_portfolio,integrated_pi_portfolio
%   SENS_INCLUDE_WEIGHT_SCENARIOS=1 : 운영목표별 AHP 가중치 민감도 포함
%   SENS_WEIGHT_GRID_STEP=0.1    : 운영목표 가중치 3축 격자 시나리오 추가
%   SENS_WEIGHT_GRID_MIN=0.1     : 격자 시나리오의 최소 기준 가중치
%   SENS_FIGURE_MODE=core        : core(논문 핵심 그림), all(전체 그림), off(그림 생략)
...
runDir = fullfile(outputRoot, ['sensitivity_analysis_matlab_' runStamp]);
...
diary(fullfile(runDir, "sensitivity_analysis_matlab.log"));
diary on;
...
for t = 1:numel(assetTypes)
    ...
    solverObjectives(t) = localSolver.objective;
